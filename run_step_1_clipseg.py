...
if os.path.exists(filename):
    with open(filename, 'r') as f:
        record_of_test = json.load(f)
        print("文件已存在，内容读取成功：", record_of_test)
else:
    record_of_test = []
    with open(filename, 'w') as f:
        json.dump(record_of_test, f)
        print("文件不存在，已创建新文件并写入初始内容。")
...
def remove_position_cue_qwen(sentence, pos_cue):
  for i in range(1):
...
def get_noun_list(sentence):
  try:
    completion = client.chat.completions.create(
        model="qwen-plus",
        messages=[
            {'role': 'system', 'content': "Here is a sentence, please extract nouns from it, return a list of nouns, divide in comma. Please give ONLY the list, but no other information."},
            {'role': 'user', 'content': sentence}],
        )
    # 获取助手的回复内容
    sentence = completion.choices[0].message.content
...
def get_judge_value_with_clipseg(image, text_prompt, labels, bbox):
  image = image.crop(bbox)
  ...
  # 加载预训练的模型和处理器
  # 使用处理器对文本和图片进行处理
  inputs = processor_of_clipseg(text=labels, images=[image] * len(labels), padding=True, return_tensors="pt")
  
  inputs = inputs.to(model_of_clipseg.device)
  # 使用模型进行预测
  outputs = model_of_clipseg(**inputs)
  ...
  torch_heat_maps = torch.sigmoid(logits)
  # logits = logits.unsqueeze(1)
  
  n = len(labels)
  # fig, axs = plt.subplots(1, n + 3, figsize=(4 * (n + 1), 4))  # 1行 n+1列
  ...
  for i, phrase in enumerate(labels):
    # 取出当前文本的热图
    heat_map = torch_heat_maps[i].squeeze().cpu()

    if heat_map.dim() == 2:
      # 如果维度是 (H, W)，需要增加 batch 和 channel 维度
      heat_map = heat_map.unsqueeze(0).unsqueeze(0)  # 变成 (1, 1, H, W)
    elif heat_map.dim() == 3:
      # 如果维度是 (C, H, W)，只需增加 batch 维度
      heat_map = heat_map.unsqueeze(0)  # 变成 (1, C, H, W)

    heat_map_resized = F.interpolate(heat_map, size=(500, 500), mode='bilinear', align_corners=False)
    heat_map_resized = heat_map_resized.squeeze().cpu().detach().numpy()  # 转为NumPy数组
    # 将热图变为 500*500
    heat_maps.append(heat_map_resized)
  general_heatmap = None
  for heatmap in heat_maps:
    if general_heatmap is None:
...
def Simple_VG(image_dir, question, min_res = 1500):
  ...
  # 定义文本提示
  labels = get_noun_list(question_without_pos)
  init_patch['heatmap'], init_patch['priority_score'] = get_judge_value_with_clipseg(image, question, labels, init_patch['bbox'])
  ...
  while(not queue.empty()): # the main loop of A_star searching
    ...
    if max(current_bbox[2] - current_bbox[0], current_bbox[3] - current_bbox[1]) >= min_res:
      for clip_suffix, clip_bbox in crop_coords:
        ...
        clip_patch['heatmap'], clip_patch['priority_score'] = get_judge_value_with_clipseg(image, question, labels, clip_bbox)
        cnt_item_queue += 1
        queue.put(Prioritize(-clip_patch['priority_score'], clip_patch))
  return related_bbox_2, max_patch['bbox'], question_without_pos
...
def judge_qwen_api(item, image_dir, image2_bbox, image3_bbox, question_without_pos):

  # 定义一个全局变量record_of_test
  ...
  # 获取item中的Answer choices
  image = Image.open(image_dir).convert('RGB')
  # 获取item中的Ground truth
  
  # 获取item中的Category
  related_bbox, pos_cue = position_cue_qwen(question, image.width, image.height)

  # 打开image_dir中的图片，并将其转换为RGB格式
  ...
  # 调用position_cue_qwen函数，获取related_bbox和pos_cue
  ...
  record_of_test.append([id, image_dir, ground_truth, category, image2_bbox, image3_bbox, question, question_without_pos])
...
for json_data in tqdm(existed_questions_box, desc="Processing questions", unit="question"):
  ...
  if os.path.exists(image_path):
    print(image_path)
    flag_go = True
    # 遍历record_of_test中的每一个json_record
    for json_record in record_of_test:
      # 打印json_record[0]
      # print("json_record[0]: ", json_record[0], ", json_data['Question_id']: ", json_data['Question_id'])
      if json_record[0] == json_data['Question_id']:
        print("Found: ", json_record[0], "\n\n\n\n")
        flag_go = False
    if flag_go == True:
      image = Image.open(image_path)
      related_bbox_2, related_bbox_3, question_without_pos = Simple_VG(image_path, json_data.get('Text'))
      judge_qwen_api(json_data, image_path, related_bbox_2, related_bbox_3, question_without_pos)
    else:
      print("Skip")
  else:
